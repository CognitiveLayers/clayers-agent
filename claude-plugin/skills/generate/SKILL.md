...
This skill is only an interface to Clayers. Claude must not author the knowledge model, rewrite Clayers XML, invent mappings, or resolve semantic drift itself. If the installed Clayers core or configured hosted Clayers service does not expose autonomous sync/generation, report that clearly instead of substituting Claude-authored specs.
...
5. Request Clayers-owned sync/generation.
   ...
   If `--hosted` is requested, or `CLAYERS_ORCHESTRATOR_URL`/`CLAYERS_API_URL` is configured, submit the current repo to the Clayers orchestrator and stream job events:
...
9. Finish with a concise report:
